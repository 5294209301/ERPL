...
public class moLineLotSection extends LotMasterSection {
	private static final Logger logger = Logger.getLogger(moLineLotSection.class);
	...
	public moLineLotSection(ADTable adTable, LotDialog parentDialog) {
		super(adTable, parentDialog);
	}
	
	public moLineLotSection(ADBase parent, ADBase child, ADTable adTable,
			MoLotDialog olld, boolean isView) {
		super(adTable, olld);
	...
	public moLineLotSection(ADTable adTable, MovementOut out,
		...
		try {
			if(lines != null && lines.size() > 0) {
				if(validateAll()) {
					...
					((MoLotDialog)parentDialog).buttonPressed(IDialogConstants.CANCEL_ID);
				}				
			}
		} catch(Exception e) {
