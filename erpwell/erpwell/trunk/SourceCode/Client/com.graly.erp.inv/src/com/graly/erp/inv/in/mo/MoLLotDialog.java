...
public class MoLLotDialog extends TitleAreaDialog {
	...
	public MoLLotDialog(Shell parentShell, MovementLine movementLine, Lot lot) {
		super(parentShell);
	...
	protected boolean validate() {
		try {
			...
			if(BigDecimal.ZERO.compareTo(inputQty) == 0) {
				setErrorMessage(String.format(Message.getString("common.largerthan"),getMovementQtyLabel(), inputQty.toString()));
				return false;
			}
			if(inputQty.compareTo(BigDecimal.ZERO) == 0) {//或者是出库后库存增加(核销时)
				setErrorMessage(String.format(Message.getString("common.largerthan"),getMovementQtyLabel(), inputQty.toString()));
				return false;
			}else if(inputQty.compareTo(BigDecimal.ONE) == 1 && inputQty.compareTo(mo.getQtyProduct()) == -1){
				return true;
		// 这里验入库数量是否大于工作令数量
			}else if(inputQty.compareTo(mo.getQtyProduct()) == 1){
				setErrorMessage("入库数量不能大于工作令订单数量");
				return false;
			}
		} catch(Exception e) {
			setErrorMessage(Message.getString("c"));
		}
		return false;
	}
